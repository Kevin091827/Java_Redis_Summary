# 一，redisTemplate
spring-data-redis 中的核心操作类是 RedisTemplate，是操作redis实现缓存的重要API

# 二，springboot2.0整合redis

## 1. springboot1.x和2.x在整合redis上的区别
...
redis配置类的编写主要解决这几个问题：

- jedis连接池配置

- 连接工厂配置

- redisTemplate自定义（设置序列化方式，设置连接工厂）
...
至此springboot2.x基于jedis连接池就配置完成
